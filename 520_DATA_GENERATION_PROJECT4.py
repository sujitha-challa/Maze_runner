...
def AStarSearch(Grid, source, destination, heuristic):
    ...
    if heuristic == 0:
        ...
        heuristicValue = ((destinationNode.position[0] - sourceNode.position[0]) + (
                    destinationNode.position[1] - sourceNode.position[1]))
        ...
        heuristicValue = np.maximum(destinationNode.position[0] - sourceNode.position[0],
                                    destinationNode.position[1] - sourceNode.position[1])
...
def FindNeighborsIn_8_Directions(currentNode):
    ...
    for newPosition in neighbours_in_8_directions:
        ...
        # Check if the node position is within the maze dimensions
        if (nodeLocation[0] > (rowCount - 1) or nodeLocation[0] < 0 or nodeLocation[1] > (columnCount - 1) or
                nodeLocation[1] < 0):
            continue
        else:
...
def FindAction(currentPosition, nextPosition):
    action = -1

    if currentPosition == nextPosition:
        return None
    for positions in neighbours_in_4_directions:
        newPosition = (currentPosition[0] + positions[0], currentPosition[1] + positions[1])
...
def CreateInputSpaceAgent1(node, nextPos):
    action = FindAction(node, nextPos)
    knownGrid = np.array(discoveredGrid)
    # currentState = [knownGrid, node]
    inputSpace.append(knownGrid)
    positions.append(node)
    outputSpace.append(action)

def CreateInputSpaceAgent3(node, nextPos):
    action = FindAction(node, nextPos)
    knownGrid = np.array(discoveredGrid_agent3)
    knownGrid_partial_sensing = np.array(discoveredGrid_agent3_partial_sensing)
    # currentState = [knownGrid, node]
    inputSpace.append(knownGrid)
    inputSpace_partial_sensing.append(knownGrid_partial_sensing)
    positions.append(node)
    outputSpace.append(action)
        ...
        for index, node in enumerate(path):
            ...
            if blindFolded:
                ...
                for newPosition in neighbours_in_4_directions:
                    ...
                    discoveredGridWithRestBlockages[nodeLocation[0]][nodeLocation[1]] = maze[nodeLocation[0]][nodeLocation[1]]
...
# RepeatedAStarSearch calls AStarSearch function repeatedly until path to goal node is discovered.
def RepeatedAStarSearch_agent1_Proj4(maze, source, destination, heuristic, blindFolded=False):
    global rowCount, columnCount, discoveredGrid, discoveredGridWithRestBlockages, processedCells, totalPlanningTime
    # A maze named discoveredGrid is created and intialized with 0(unblocked) at start and end positions.
    # And with '-1' at rest of the positions, indicating the status as unknown.
    # Whenever a node is discovered that node position will be updated.
    ...
    destinationNode = Node(None, tuple(destination))
    resultPath = []
    global solvable, inputSpace, positions, outputSpace
    inputSpace = []
    positions = []
    outputSpace = []
    ...
    while pathFound == False:
        ...
        for index, node in enumerate(path):
            ...
            if currentNode.position == destinationNode.position:
                pathFound = True
                resultPath.append(node)
                # CreateInputSpace(node, destinationNode.position)
                break
            # else:
            # CreateInputSpace(node, path[index+1])
            ...
            CreateInputSpaceAgent1(node, path[index + 1])
...
# RepeatedAStarSearch calls AStarSearch function repeatedly until path to goal node is discovered.
def RepeatedAStarSearch_Agent3_Proj4(maze, source, destination, heuristic):
    global rowCount, columnCount, discoveredGrid, discoveredGridWithRestBlockages, processedCells, totalPlanningTime, discoveredGrid_agent3, discoveredGrid_agent3_partial_sensing
    # A maze named discoveredGrid is created and intialized with 0(unblocked) at start and end positions.
    # And with '-1' at rest of the positions,  indicating the status as unknown.
    # 1 indicated blocked, 0 indicated unblocked, -1 indicates value is unknown
    # Whenever a node is discovered that node position will be updated.
    discoveredGrid_agent3_partial_sensing = [[-1 for i in range(columnCount)] for j in range(rowCount)]
    discoveredGrid_agent3 = [[0 for i in range(columnCount)] for j in range(rowCount)]
    # discoveredGrid_agent3[0][0] = 0
    # discoveredGrid_agent3[rowCount-1][columnCount-1] = 0
    discoveredGrid = [[0 for i in range(columnCount)] for j in range(rowCount)]
    # discoveredGrid[0][0] = 0
    # discoveredGrid[rowCount-1][columnCount-1] = 0
    discoveredGridWithRestBlockages = [[1 for i in range(columnCount)] for j in range(rowCount)]
    ...
    while pathFound == False:
        ...
        for index, node in enumerate(path):
            ...
            discoveredGrid_agent3_partial_sensing[path[index][0]][path[index][1]] = currentNode.C
            CreateInputSpaceAgent3(node, path[index + 1])
            # maze[path[index + 1][0]][path[index + 1][1]]

            InferenceOnSingleNodeForAgent3(currentNode, neighbourNodes)
            valueOfNextNodeInGivenPath = maze[path[index + 1][0]][path[index + 1][1]]
            if discoveredGrid[path[index + 1][0]][path[index + 1][1]] == 1:
                source = path[index]
                discoveredGrid_agent3[path[index + 1][0]][path[index + 1][1]] = valueOfNextNodeInGivenPath
                break
    ...
    while pathFound == False:
        ...
        for index, node in enumerate(path):
            ...
            for pathPosition in nodesWithUndeterminedNeighbours.keys():
                ...
                InferenceOnSingleNodeForAgent4(previousNode, neighbourNodesofPreviousNode,
                                               nodesWithUndeterminedNeighbours, inferredNodes)
            ...
            discoveredGrid[path[index + 1][0]][path[index + 1][1]] = valueOfNextNodeInGivenPath
            discoveredGridWithRestBlockages[path[index + 1][0]][path[index + 1][1]] = valueOfNextNodeInGivenPath
            if currentNode.position in nodesWithUndeterminedNeighbours and path[index + 1] in \
                    nodesWithUndeterminedNeighbours[currentNode.position]:
...
def InferenceRulesForAgent4(nodesWithUndeterminedNeighbours, nodeStaus):
    ...
    if len(undeterminedNodes) >= 2:
        ...
        commonUndeterminedNodes = (nodesWithUndeterminedNeighbours[latestNodePosition]).intersection(
            nodesWithUndeterminedNeighbours[previousNodePosition])
        if len(commonUndeterminedNodes) > 0:
            ...
            if undeterminedBlocksLatest > undeterminedCellsNotCommon:
                ...
                if undeterminedEmptyCellsLatest > undeterminedCellsNotCommon:
                    ...
                    previousNodeStaus = nodeStaus[previousNodePosition]
                    undeterminedEmptyCellsPrevious = previousNodeStaus['N'] - previousNodeStaus['C'] - \
                                                     previousNodeStaus['E']
...
def PlotPerformanceComparisionForAgents1_2_3():
    ...
    for p in np.arange(0, maxProb + 0.01, 0.033):
        ...
        averagePathLengthInFinalDiscoveredGridforAgent1.append(
            sum(pathLengthInFinalDiscoveredGridforAgent1) / len(pathLengthInFinalDiscoveredGridforAgent1))
        averagePathLengthInFinalDiscoveredGridforAgent2.append(
            sum(pathLengthInFinalDiscoveredGridforAgent2) / len(pathLengthInFinalDiscoveredGridforAgent2))
        averagePathLengthInFinalDiscoveredGridforAgent3.append(
            sum(pathLengthInFinalDiscoveredGridforAgent3) / len(pathLengthInFinalDiscoveredGridforAgent3))
...
# Generating random maze for dimension 101
# with probabilities ranging from 0 to 1 in interval of 0.033
def DataGenerationForML_agent_1():
    global rowCount, columnCount, discoveredGrid, discoveredGridWithRestBlockages, processedCells, dimen
    dimen = 10
    iterations = 100
    # po value from question 4
    p = 0.2
    ...
    for y in range(iterations):
        solvableGrid = False
        while solvableGrid == False:
            maze = RandomGridGenerator(p, dimen)
            rowCount, columnCount = np.shape(maze)
            start = [0, 0]  # starting position
            end = [rowCount - 1, columnCount - 1]  # ending position
            startTime = time.time()  # Start time
            pathInFullGrid = AStarSearch(maze, start, end, heuristic)
            endTime = time.time()  # end time
            AstartTime = endTime - startTime
            solvableGrid = solvable
            if solvableGrid:
                path1 = RepeatedAStarSearch_agent1_Proj4(maze, start, end, heuristic, False)
                PathForAgent1.append(path1)
                pickleKG.extend(inputSpace)
                picklePosition.extend(positions)
                pickleAction.extend(outputSpace)
                pickleIndex.extend(np.full((len(positions), 1), y + 1, dtype=int))
                pickleMaze.extend([maze])
                pickleAstarTime.append(AstartTime)

                """
                dict = {'Index' : y+1, 'KnowledgeGrid': inputSpace, 'position': positions, 'action': outputSpace, 'maze': [maze]}
                df = pd.DataFrame(dict)
                if y == 0:
                    df.to_csv('grids_agent1_dim100_count100000.csv', mode='w', index=False)
                else:
                    df.to_csv('grids_agent1_dim100_count100000.csv', mode='a', index=False, header=False)
                #df.to_csv('grids.csv', mode='a',index=False, header=False)
                """

    dict = {'Index': pickleIndex, 'KnowledgeGrid': pickleKG, 'position': picklePosition, 'action': pickleAction,
            'maze': pickleMaze, 'AstarTime': pickleAstarTime}
    with open("grids_agent1_dim10_count100.pkl", "wb") as f:
        pickle.dump(dict, f)

# This is code for comparision of Agent 1, 2, 3 - comparing trajectory length, total planning time, tajectory length for path in final discoverd girdworld
# Generating random maze for dimension 101
# with probabilities ranging from 0 to 1 in interval of 0.033
def DataGenerationForML_agent_3():
    global rowCount, columnCount, discoveredGrid, discoveredGridWithRestBlockages, processedCells, dimen
    dimen = 10
    iterations = 100
    # po value from question 4
    ...
    for y in range(iterations):
        ...
        while solvableGrid == False:
            ...
            solvableGrid = solvable
            if solvableGrid:
                path1 = RepeatedAStarSearch_Agent3_Proj4(maze, start, end, heuristic)
                PathForAgent1.append(path1)
                pickleKG.extend(inputSpace)
                pickleKG_partial_sensing.extend(inputSpace_partial_sensing)
                picklePosition.extend(positions)
                pickleAction.extend(outputSpace)
                pickleIndex.extend(np.full((len(positions), 1), y + 1, dtype=int))
                pickleMaze.extend([maze])
    ...
    dict = {'Index': pickleIndex, 'KnowledgeGrid': pickleKG, 'KnowledgeGridPS': pickleKG_partial_sensing,
            'position': picklePosition, 'action': pickleAction, 'maze': pickleMaze, 'AstarTime': pickleAstarTime}
    with open("grids_agent3_dim10_count100.pkl", "wb") as f:
        pickle.dump(dict, f)
...
if __name__ == '__main__':
    ...
    dim = 0
    discoveredGrid = []
    discoveredGrid_agent3 = []
    discoveredGrid_agent3_partial_sensing = []
    ...
    DataGenerationForML_agent_1()
    DataGenerationForML_agent_3()
    ...
    # maze = RandomGridGenerator(0.25, 5)
    ...
    startTime = time.time()  # Start time
    # RepeatedAStarSearch_Proj4(maze, source, destination, heuristic, True)
    endTime = time.time()  # End time
    """
